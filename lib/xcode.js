...
import { execRun, osIsDarwin, run, } from './common.js';
export class XcodeInfo {
    ...
    async _readDefaultsForKey(key) {
        const result = await execRun(`Xcode: Reading defaults for ${key}...`, 'defaults', ['read', `${this.path}/Contents/Info`, key]);
...
XcodeInfo.installedUnderApplicationsDirectory = (() => {
    let installedXcodeApplicationsUnderApplicationsDirectory = void (0);
    return () => {
...
XcodeInfo.all = (() => {
    ...
    return async () => {
        ...
        const result = new Map();
        const commandResult = await execRun("Searching all Xcode applications...", 'mdfind', ['kMDItemCFBundleIdentifier == "com.apple.dt.Xcode"'], { ignoreReturnCode: true });
...
XcodeInfo.latest = (() => {
    ...
    return async () => {
        ...
        const result = await run("Determining the latest Xcode...", async () => {
            let currentLatest = void (0);
...
XcodeInfo.forSwift = (() => {
    ...
    return async (version) => {
        ...
        const foundXcode = await run('Check whether or not Swift ' + version + ' is already installed.', async () => {
