...
export class Swiftly extends SwiftInstaller {
    ...
    static async _setUp() {
        ...
        if (osIsDarwin) {
            await execRun('Installing swiftly...', 'installer', [
                '-pkg', localPackagedBinaryFilename,
                '-target', 'CurrentUserHomeDirectory',
            ], {
                cwd: Swiftly.directory,
            });
            await execRun("Initializing swiftly...", `${os.homedir()}/.swiftly/bin/swiftly`, [
                'init',
                '--skip-install',
            ]);
        }
        else {
            await execRun("Unarchiving swiftly...", "tar", [
                'zxf', localPackagedBinaryFilename
            ], {
                cwd: Swiftly.directory
            });
            await execRun("Initializing swiftly...", `${Swiftly.directory}/swiftly`, [
                'init',
                ' --skip-install',
            ]);
        }
