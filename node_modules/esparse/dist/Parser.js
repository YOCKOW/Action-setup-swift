...
let _AST = require('./AST.js');
const AST = _AST;
let _Scanner = require('./Scanner.js');
let _Transform = require('./Transform.js');
...
class Parser {
  constructor(input, options) {
    options = options || {};
    let scanner = new _Scanner.Scanner(input, options.offset);
    this.onASI = options.onASI || null;
    this.scanner = scanner;
    this.input = input;
    this.peek0 = null;
    this.peek1 = null;
    this.tokenStash = new _Scanner.Scanner();
  ...
  parseAnnotations(context) {
    while (this.peek(context) === '@') this.annotations.push(this.Annotation());
  }
  ...
  Expression(noIn) {
    ...
    while (this.peek('div') === ',') {
      this.read();
      if (list === null) expr = this.node(new AST.SequenceExpression(list = [expr]), expr.start);
      if (this.peek() === ')') {
  ...
  AssignmentExpression(noIn, allowSpread) {
    ...
    return this.node(new AST.AssignmentExpression(node, this.read(), this.AssignmentExpression(noIn)), start);
  }
    ...
    while (!exit) {
      ...
      switch (token.type) {
        ...
        case 'TEMPLATE':
          ...
          expr = this.node(new AST.TaggedTemplateExpression(expr, this.TemplateExpression()), start);
          break;
        default:
          if (isSuper) this.fail();
          exit = true;
  ...
  Identifier(isVar) {
    ...
    let node = this.node(new AST.Identifier(token.value, isVar ? 'variable' : ''), token.start, token.end);
  ...
  NumberLiteral() {
    ...
    let node = this.node(new AST.NumberLiteral(token.number, token.numberSuffix), token.start, token.end);
    if (token.strictError) this.addStrictError(token.strictError, node);
    return node;
  }
  ...
  TemplatePart() {
    ...
    let node;
    node = this.node(new AST.TemplatePart(token.value, this.scanner.rawValue(token.start + 1, token.end - (end ? 1 : 2)), end), token.start, token.end);
    if (token.strictError) this.addStrictError(token.strictError, node);
    return node;
  }
  ...
  RegularExpression() {
    ...
    return this.node(new AST.RegularExpression(token.value, token.regexFlags), token.start, token.end);
  }
  ...
  PropertyDefinition() {
    ...
    switch (this.peekAt('name', 1)) {
      case '=':
        this.unpeek();
        node = this.node(new AST.PatternProperty(this.Identifier(true), null, (this.read(), this.AssignmentExpression())), start);
        this.addInvalidNode('Invalid property definition in object literal', node);
        return node;
      case ',':
      case '}':
        this.unpeek();
        return this.node(new AST.PropertyDefinition(this.Identifier(true), null), start);
    }
    name = this.PropertyName();
    if (this.peek('name') === ':') {
      return this.node(new AST.PropertyDefinition(name, (this.read(), this.AssignmentExpression())), start);
    }
  ...
  WithStatement() {
    ...
    this.read('with');
    this.read('(');
    let node = this.node(new AST.WithStatement(this.Expression(), (this.read(')'), this.Statement())), start);
  ...
  StatementListItem() {
    switch (this.peek()) {
      ...
      case 'IDENTIFIER':
        ...
        if (this.peekAsync() === 'function') return this.FunctionDeclaration();
        break;
    }
...
mixin(Parser, _Transform.Transform, _Validate.Validate);