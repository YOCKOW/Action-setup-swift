...
exports.ScopeResolver = undefined;
...
class ScopeResolver {
  ...
  visit(node, kind) {
    ...
    else _AST.forEachChild(node, (n) => this.visit(n, kind));
  }
  ...
  Script(node) {
    ...
    _AST.forEachChild(node, (n) => this.visit(n, 'var'));
    this.popScope();
  }
  ...
  Block(node) {
    this.pushScope('block', node);
    _AST.forEachChild(node, (n) => this.visit(n));
    this.popScope();
  }
  ...
  VariableDeclaration(node) {
    _AST.forEachChild(node, (n) => this.visit(n, node.kind));
  }

  ImportDeclaration(node) {
    _AST.forEachChild(node, (n) => this.visit(n, 'const'));
  }
